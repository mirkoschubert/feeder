...
const Promise = require('promise'),
      ...
      MetasPromise = require(__dirname + '/metaspromise'),
      chalk = require('chalk'),
...
Feeds.prototype.updateFeeds = function () {
  ...
  fs.readFile(self.file, 'utf8', function(err, data) {
    if (err) throw err;
    self.data = JSON.parse(data);
    var progress = new Progress('[:bar]', { total: self.data.feeds.length });
    for (var i = 0; i < self.data.feeds.length; ++i) {
...
Feeds.prototype.loadFeed = function (url, count) {
  ...
  fp.on('readable', function() {
    ...
    var meta = this.meta;
    var item;
/*    var len = parseInt(feedparser.headers['content-length'], 10);
    var bar = new Progress('Loading Content |:bar| :percent', {
      complete: '\u2593',
      incomplete: '\u2591',
      width: 20,
      total: len
  ...
  fp.on('end', function() {
    ...
    for (var i = 0; i < self.items.length; i++) {
      promises.push(MetasPromise(self.items[i], ['description', 'og:description']));
    }
