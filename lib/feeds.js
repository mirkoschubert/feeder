...
/**
 * Feeds.save (asynchronous)
 * @param  {String}   file  Filename
 * @param  {Object}   data  All the relevant data
 * @return {Promise}        Everything's fine
 */
Feeds.prototype.save = function (file, data) {
...
Feeds.prototype.generateFilename = function (url) {

  if (!url || url.length === 0) {
    throw new Error('URL is empty');
  } else {
    let u = new URL(url);
    let file = u.hostname.split('.')[0] + '.json';
    return file;
  }
}
...
Feeds.prototype.load = function (url, count) {
  ...
  console.log(self.generateFilename(url));
